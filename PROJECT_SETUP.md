...
| 当前自适应组合版 | 16/46 | 1744 | 1.69 px | 1737 |

当前自适应组合版保留严格的双视图点过滤；当新相机在 4 px PnP 阈值下没有足够内点时，
仅对该相机逐步尝试 6 px 和 8 px。该策略把注册覆盖率从 13/46 提升到 16/46，同时保持
1.69 px 的低平均重投影误差。它仍是局部重建，后续需继续改进增量注册策略以恢复完整
场景覆盖率。
